...
class TFBMI:
    ...
    def ct_2(self, t):
        ...
        if t == 0:
            ...
            Ct = ((2*gamma(2*self.H))/(2*self.lambd)**(2*self.H)) - (abs(t)**self.H) * ((2*gamma(self.H+0.5))/np.sqrt(np.pi)) * (1/((2*self.lambd)**(self.H))) * kv(self.H, abs(self.lambd*t))
        return Ct
    ...
    def generate_and_save_cov(self):
        ...
        np.savetxt(f"cov_matrices_tfbm1/H_{h_str}_l_{l_str}_T_{str(self.T)}_N_{str(self.N)}.txt", sigma)
        return sigma

    def load_cov_matrix(self):
        h_str = str(self.H).replace('.', '_')
        l_str = str(self.lambd).replace('.', '_')
        filename = f"cov_matrices_tfbm1/H_{h_str}_l_{l_str}_T_{str(self.T)}_N_{str(self.N)}.txt"
