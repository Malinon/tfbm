...
def quadratic_stat_test(stat_matrix:np.ndarray, test_statistic:float, cov_matrix:np.ndarray, monte_carlo_steps:int) -> float:
    """ Performs a statistical test based on a quadratic form statistic described in https://doi.org/10.1063/5.0044878

    *Null hypothesis:* trajectory is generated by a centered Gaussian process with covariance matrix cov_matrix

    *Alternative hypothesis:* trajectory is not a centered Gaussian process with covariance matrix cov_matrix

    Parameters
    ----------
    stat_matrix : np.ndarray
        The matrix defining the quadratic form of the test statistic.
    test_statistic : float
        The value of the test statistic computed from the trajectory.
    cov_matrix : np.ndarray
        The covariance matrix of the trajectory under the null hypothesis.
    monte_carlo_steps : int
        The number of Monte Carlo samples to use for estimating the p-value.
    Returns
    -------
    p_value : float
...
def _general_quadratic_form_test(matrix_gen, trajectory, tfbm_type, H_0, lambda_0, lag, T, monte_carlo_steps):
    ...
    p_value = quadratic_stat_test(stat_matrix, test_statistic, cov_matrix, monte_carlo_steps)
    return p_value
...
def avcf_test(trajectory:np.ndarray, tfbm_type:str, H_0:float, lambda_0:float, lag:int, T:float, monte_carlo_steps:int=1000) -> float:
    """
...
def dma_test(trajectory:np.ndarray, tfbm_type:str, H:float, lambd:float, lag:int, T:float, monte_carlo_steps:int=1000) -> float:
    """
    ...
    *Alternative hypothesis:* trajectory is not TFBM of type tfbm_type with parameters H and lambd (λ)
    ...
    return quadratic_stat_test(stat_matrix, test_statistic, _dma_covariance(tfbm_type, H, lambd, N, T, lag), monte_carlo_steps)
