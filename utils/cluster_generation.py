...
def generate_trajectories_for_parameter(H, lambd, n_trajectories):
    print(f'Started {OUTPUT_DIRETORY}/TYPE_{TFBM_TYPE}_T_{T}_H_{H}_lambda_{lambd}.npy')
    ...
    try:
        ...
        print(f'{OUTPUT_DIRETORY}/TYPE_{TFBM_TYPE}_T_{T}_H_{H}_lambda_{lambd} FAILED')
        return
    np.save(f'{OUTPUT_DIRETORY}/TYPE_{TFBM_TYPE}_T_{T}_H_{H}_lambda_{lambd}.npy', trajectories)
    print(f'Done {OUTPUT_DIRETORY}/TYPE_{TFBM_TYPE}_T_{T}_H_{H}_lambda_{lambd}.npy')
    sys.stdout.flush()

def worker(params):
    H, lambd = params
    if not os.path.exists(f'{OUTPUT_DIRETORY}/TYPE_{TFBM_TYPE}_T_{T}_H_{H}_lambda_{lambd}.npy'):
